...
enum ResultadoComparacion_Minas {
  MAYOR_m,
  IGUAL_m,
  MENOR_m
};
...
typedef Minas DatoJ_m;

/* Tipo de Estructura de los Nodos de la Lista. */
struct NodoLista_Minas {
    DatoJ_m dato; // dato almacenado
    NodoLista_Minas* sgte; // puntero al siguiente
};

/* Tipo de Puntero a los Nodos de la Lista, el cual se usa para recorrer
   la Lista y acceder a sus Datos. */
typedef NodoLista_Minas* PtrNodoLista_Minas;


/* Tipo de Estructura de la Lista */
struct Lista_Minas{
    PtrNodoLista_Minas primero;      // puntero al primer nodo de la lista
};
...
/*
  ...
  return puntero al nodo adicionado.
*/
PtrNodoLista_Minas adicionarDespues_Minas(Lista_Minas &lista, DatoJ_m, PtrNodoLista_Minas ptrNodo);
